/**********************************************************************
 Copyright (c) 2020-2025, Unitree Robotics.Co.Ltd. All rights reserved.
***********************************************************************/
#ifndef LOCOMOTION_H
#define LOCOMOTION_H

#include <iostream>
#include <sstream>
#include <fstream>
#include <string>
#include <vector>
#include <algorithm>
#include <optional>
#include <chrono>
#include <iomanip>
#include <thread>

#include <onnxruntime_cxx_api.h>
#include "Utilities/getWorkingDir.h"
#include "FSM/FSMState.h"
// #include <MNN/Interpreter.hpp>
#include "Utilities/yaml_parser.h"

class State_locomotion : public FSMState
{
public:
    State_locomotion(CtrlComponents *ctrlComp);
    ~State_locomotion() {}
    void enter();
    void run();
    void exit();
    FSMStateName checkChange();

private:
    // std::shared_ptr<MNN::Interpreter> net = nullptr;
    // 使用MNN模型
    // MNN::Session* session = nullptr;
    // MNN::Tensor *obs_tensor = nullptr;
    // MNN::Tensor *act_tensor = nullptr;

    // 使用ONNX模型
    std::unique_ptr<Ort::Env> env;
    std::unique_ptr<Ort::Session> session;  // 使用Ort::Session
    Ort::SessionOptions session_options;

    // 改为ONNX Runtime的类型
    Ort::Value input_tensor{nullptr};
    std::vector<float> input_tensor_values;
    std::vector<int64_t> input_shape;
    std::string input_name;

    Ort::Value output_tensor{nullptr};
    std::vector<float> output_tensor_values;
    std::vector<int64_t> output_shape;
    std::string output_name;

    std::vector<const char*> input_names;  // 注意：这里是const char*
    std::vector<const char*> output_names; // 注意：这里是const char*

    Eigen::VectorXf obs;
    Eigen::VectorXf stiffness;
    Eigen::VectorXf damping;
    Eigen::VectorXf obs_mean;
    Eigen::VectorXf obs_scales;
    Eigen::VectorXf obs_scaled;
    Eigen::VectorXf act_mean;
    Eigen::VectorXf act_scales;
    Eigen::VectorXf act_scaled;
    Eigen::VectorXf act_temp;
    Eigen::VectorXf obs_current;
    Eigen::VectorXf act_prev;

    long long _startTime;
    bool threadRunning = true;
    std::thread _thread;
    void inference();

    // frame stacking for model input
    int frame_stack_size = 1; // number of frames to stack (will set to 66 in loadPolicy)
    int stacked_input_size = 0; // num_observations * frame_stack_size
    std::vector<float> stacked_obs; // flattened stacked observations

    Vec12<double> _q;
    Vec12<double> _qd;
    RotMat<double> _B2G_RotMat, _G2B_RotMat;

    void loadPolicy();
    YAML::Node env_cfg;
    int num_observations,num_actions;
    float dt;
    pthread_mutex_t write_cmd_mutex;
    YamlParser yamlParam_;
    float counter = 0;
    float phase = 0;
    
    float obsScaleAngularVelocity = 0.2;
    float obsScaleLinearVelocity = 0.1; 
    
    // optional dumping of policy inputs for cross-check with sim2sim
    bool dump_policy_input = false;
    std::string input_file_path = "./policy_input_a1.csv";
    std::ofstream input_file;

    bool dump_policy_output = false;
    std::string output_file_path = "./policy_output_a1.csv";
    std::ofstream output_file;

private:
    void CreateDumpFile(const char* dump_env);    
};
#endif 
