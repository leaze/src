...
class ArmKinematics:
    def __init__(self, is_left=True):
        ...
        self.joint_params = [
            ...
            # shoulder_yaw (关节2)
            {"pos": np.array([0, 0, -0.1025]), "rot_axis": np.array([0, 0, 1]), "rot_rpy": np.array([0, 0, 0])},  # z轴
            # elbow_pitch (关节3)
            {"pos": np.array([0.02, 0, -0.1975]), "rot_axis": np.array([0, 1, 0]), "rot_rpy": np.array([0, 0, 0])},  # y轴
            # elbow_yaw (关节4)
            {"pos": np.array([-0.02, 0, -0.0543]), "rot_axis": np.array([0, 0, 1]), "rot_rpy": np.array([0, 0, 0])},  # z轴
            # wrist_pitch (关节5)
            {
            ...
            # wrist_roll (关节6)
            {"pos": np.array([5.0485e-05, 0, -0.02]), "rot_axis": np.array([1, 0, 0]), "rot_rpy": np.array([0, 0, 0])},  # x轴
        ]
    ...
    def inverse_kinematics(
        self, target_position, target_quaternion=None, initial_angles=None, orientation_weight=1.0, use_rotation_matrix=False
    ):
...
if __name__ == "__main__":
    ...
    recovered_left_joints = left_arm.inverse_kinematics(
        left_pos, target_quaternion=left_quat, initial_angles=left_joints, use_rotation_matrix=False
    )

    # 右臂：使用位置和方向（旋转矩阵方法）
    recovered_right_joints = right_arm.inverse_kinematics(
        right_pos, target_quaternion=right_quat, initial_angles=right_joints, use_rotation_matrix=True
    )
